      ...
      >
        ...
        >
          ...
          <div class="flex flex-row justify-start">
            <a
              :href="appConfig.socialMedia.youtube"
          ...
          <div
            class="absolute inset-0 w-full h-1/2 mt-auto bg-gradient-to-t from-black/70 -z-10"
          ></div>
...
const description = ref(props.description ?? '<b>E como fazemos isso?</b> <br class="md:hidden lg:hidden xl:hidden" /> <i>Através dos nossos três pilares:</i>')

const activeImage = ref<HTMLDivElement | null>(null);
const width = ref(0);


const activeCard = ref<{ image: string; id: keyof typeof options }>({
...
function changeActiveImage(id: keyof typeof options) {
  const timeline = gsap.timeline();

  const direction = width.value <= 1024 ? [0, -100] : [-100, 0];

  if (activeImage.value) {
    timeline.to(activeImage.value, {
      xPercent: direction[0],
      yPercent: direction[1],
      duration: 0.5,
      ease: "sine",
      onComplete: () => {
        activeCard.value.id = id;
        activeCard.value.image = options[id].image;
      },
    });
    timeline.to(activeImage.value, {
      xPercent: 0,
